...
- Runtime helper alias detection recognizes:
  ...
  - defaulted object destructure aliases (e.g. `const { c: cache = fallback } = runtime`, `const { ['\\x63']: c = fallback } = runtime`, `const { c = fallback } = require(...)`, `const runtime = module['\\x72equire'](...); const { c = fallback } = runtime`)
