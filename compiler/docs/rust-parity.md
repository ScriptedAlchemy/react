...
- Runtime helper alias detection recognizes:
  ...
  - defaulted object destructure aliases (e.g. `const { c: cache = fallback } = runtime`, `const { c = fallback } = require(...)`, `const { c = fallback } = module['\\x72equire'](...)`, `const { ['\\x63']: cache = fallback } = runtime`)
