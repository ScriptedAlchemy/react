...
use crate::{
    emit::{emit_module, emit_script},
    helpers::span_to_location,
    parse::parse_syntax_error_reason,
    placeholder::count_runtime_helper_imports,
    placeholder_apply::{apply_placeholder_compilation_to_module, apply_placeholder_compilation_to_script},
    react_detect::{
        collect_placeholder_transform_candidate_names_for_module,
        collect_placeholder_transform_candidate_names_for_script, collect_react_functions_in_module,
        collect_react_functions_in_script,
    },
    react_fn::{
    ...
    runtime_scan::{
        runtime_memo_callee_scan_for_module, runtime_memo_callee_scan_for_script,
...
fn compile_script_output(
    ...
    let mut placeholder_transformed_functions = if options.apply_placeholder_transforms {
        apply_placeholder_compilation_to_script(script, &react_functions)
    } else {
        Vec::new()
    };
    let transformed_count = placeholder_transformed_functions.len();
    ...
    Ok((metadata, emit_script(cm, comments, script)?))
}
