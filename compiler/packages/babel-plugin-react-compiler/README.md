...
- Runtime helper alias reuse supports CommonJS/global patterns including
  ...
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\f\t\n\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign vertical-tab+line-feed+form-feed+carriage-return+tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\n\f\r\t16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign vertical-tab+line-feed+form-feed+tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\n\f\t\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign vertical-tab+line-feed+carriage-return+form-feed+tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\n\r\f\t16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign vertical-tab+line-feed+carriage-return+tab+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\n\r\t\f16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign vertical-tab+line-feed+tab+form-feed+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\n\t\f\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign vertical-tab+line-feed+tab+carriage-return+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\v\n\t\r\f16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+vertical-tab+form-feed+carriage-return+tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\v\f\r\t16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+vertical-tab+form-feed+tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\v\f\t\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+vertical-tab+carriage-return+form-feed+tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\v\r\f\t16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+vertical-tab+carriage-return+tab+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\v\r\t\f16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+vertical-tab+tab+form-feed+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\v\t\f\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+vertical-tab+tab+carriage-return+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\v\t\r\f16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+form-feed+carriage-return+tab+vertical-tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\f\r\t\v16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+form-feed+carriage-return+vertical-tab+tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\f\r\v\t16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+form-feed+tab+carriage-return+vertical-tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\f\t\r\v16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+form-feed+tab+vertical-tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\f\t\v\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+form-feed+vertical-tab+carriage-return+tab whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\f\v\r\t16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+form-feed+vertical-tab+tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[(+1 && 'require')]((+'+\n\f\v\t\r16' || 'react/compiler-runtime'))`).
  Signed decimal numeric-string coercions with post-sign line-feed+tab+carriage-return+form-feed+vertical-tab whitespace
  also fold as `NaN`
  ...
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\f\t\n\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign vertical-tab+line-feed+form-feed+carriage-return+tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\n\f\r\t16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign vertical-tab+line-feed+form-feed+tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\n\f\t\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign vertical-tab+line-feed+carriage-return+form-feed+tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\n\r\f\t16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign vertical-tab+line-feed+carriage-return+tab+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\n\r\t\f16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign vertical-tab+line-feed+tab+form-feed+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\n\t\f\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign vertical-tab+line-feed+tab+carriage-return+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\v\n\t\r\f16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+vertical-tab+form-feed+carriage-return+tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\v\f\r\t16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+vertical-tab+form-feed+tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\v\f\t\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+vertical-tab+carriage-return+form-feed+tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\v\r\f\t16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+vertical-tab+carriage-return+tab+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\v\r\t\f16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+vertical-tab+tab+form-feed+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\v\t\f\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+vertical-tab+tab+carriage-return+form-feed whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\v\t\r\f16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+form-feed+carriage-return+tab+vertical-tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\f\r\t\v16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+form-feed+carriage-return+vertical-tab+tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\f\r\v\t16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+form-feed+tab+carriage-return+vertical-tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\f\t\r\v16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+form-feed+tab+vertical-tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\f\t\v\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+form-feed+vertical-tab+carriage-return+tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\f\v\r\t16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+form-feed+vertical-tab+tab+carriage-return whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\f\v\t\r16'}\` || 'react/compiler-runtime')))`).
  Signed decimal template-string numeric coercions with post-sign line-feed+tab+carriage-return+form-feed+vertical-tab whitespace
  also fold as `NaN`
  (e.g. `module[((+\`${'1'}\` && 'require'))](((+\`${'+\n\t\r\f\v16'}\` || 'react/compiler-runtime')))`).
